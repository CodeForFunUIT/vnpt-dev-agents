...
async function loadSession(issueKey) {
...
export function registerSessionTools(server) {
    ...
    server.tool("save_session", "Lưu context hiện tại của task để tiếp tục ở phiên chat sau. " +
    ...
    server.tool("load_session", "Khôi phục context đã lưu của một task. " +
        ...
        const age = Math.floor((Date.now() - new Date(session.updatedAt).getTime()) / 3600000);
        return {
            content: [{
                    type: "text",
                    text: [
