...
// ── Tool registration ──────────────────────────
export function registerGwtTools(server) {
    server.tool("generate_gwt_description", "Trả về description hiện tại và hướng dẫn để Claude tự sinh GWT chuẩn. " +
...
function formatValidation(issueKey, r) {
    ...
    if (r.vague_phrases.length > 0) {
        ...
        for (const vp of r.vague_phrases) {
            lines.push(`\n### ❌ "${vp.phrase}"`, `**Tại sao mơ hồ:** ${vp.why_vague}`, `**Sửa thành:** ${vp.better_version}`);
        }
        lines.push("");
    }
    // Scenarios còn thiếu
    ...
    // Action items
    if (!r.ai_ready) {
        lines.push("**Bước tiếp theo:**", "1. Dùng `generate_gwt_description` để AI tự sinh lại description chuẩn", "2. Hoặc tự sửa theo các điểm trên rồi validate lại");
    }
    else {
        lines.push("**Bước tiếp theo:**", "1. Dùng `detect_files_from_task` để tìm file liên quan", "2. Dùng `evaluate_task_complexity` để ước tính effort", "3. Bắt đầu implement!");
    }
    return lines.join("\n");
}
