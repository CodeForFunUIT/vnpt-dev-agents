...
export function registerKickoffTools(server) {
    server.tool("task_kickoff", "Entry point cho mọi task. Đọc Jira task, phân tích nhanh, " +
        ...
        const assignee = fields.assignee?.displayName ?? "Chưa assign";
        ...
        const commentsAfterUpdate = comments
            .filter(c => new Date(c.created) > updatedDate);
        const changeSignals = comments
        ...
        const hasSecurityConcern = securityKeywords.some(kw => `${summary} ${description}`.toLowerCase().includes(kw));
        // Sub-task check
        ...
        const descOptions = descAction === "poor" ? [
        ...
        const warnings = [];
        if (hasDriftRisk) {
            warnings.push(`⚠️ **Drift risk:** Task ${ageInDays} ngày tuổi, có ${commentsAfterUpdate.length} comments mới ` +
