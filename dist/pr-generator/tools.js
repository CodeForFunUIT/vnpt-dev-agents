...
// ─────────────────────────────────────────────
export function registerPRTools(server) {
    server.tool("generate_pr_description", "Tự động sinh PR description từ Jira task + git diff + commits. " +
        ...
        let currentBranch = "";
        try {
            const { stdout: branchOut } = await execAsync("git rev-parse --abbrev-ref HEAD", { cwd: projectRoot });
            currentBranch = branchOut.trim();
            const { stdout: diffOut } = await execAsync(`git diff --name-status ${baseBranch}...HEAD`, { cwd: projectRoot });
            changedFiles = diffOut.trim().split("\n").filter(Boolean);
            const { stdout: logOut } = await execAsync(`git log ${baseBranch}..HEAD --oneline --no-merges -20`, { cwd: projectRoot });
...
function extractScenarioNames(desc) {
    return (desc.match(/^### Scenario\s*\d*[:.]\s*(.+)/gm) ?? [])
        .map(m => m.replace(/^### Scenario\s*\d*[:.]\s*/, "").trim());
}
function extractSection(desc, section) {
    const lines = desc.split("\n");
    const result = [];
    let inSection = false;
    for (const line of lines) {
