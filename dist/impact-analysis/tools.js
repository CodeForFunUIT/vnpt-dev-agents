...
export function registerImpactTools(server) {
    server.tool("analyze_impact", "Phân tích impact khi sửa file: ai import file này? Module nào bị ảnh hưởng? " +
        ...
        for (const filePath of filePaths) {
            ...
            const impacts = await findReverseImports(relativePath, projectRoot, importPattern, depth, profile.extensions);
            allImpacts.push({
        ...
        // Per-file impact
        for (const impact of allImpacts) {
            lines.push(`## 📄 \`${impact.sourceFile}\``, "");
            if (impact.importedBy.length === 0) {
                lines.push("_Không có file nào import file này_ (leaf node)", "");
                continue;
            }
            lines.push(`| File | Depth | Type |`, `|---|---|---|`, ...impact.importedBy.map(d => `| \`${d.file}\` | ${d.depth} | ${categorizeFile(d.file, profile.name)} |`), "");
        }
        // Module summary
        if (moduleMap.size > 0) {
            lines.push("## 📦 Modules ảnh hưởng", "", ...Array.from(moduleMap.entries()).map(([mod, files]) => `- **${mod}** (${files.length} files)`), "");
        }
        // Recommendations
        ...
        const testFiles = [...allAffected].filter(f => f.includes(".spec.") || f.includes(".test.") || f.includes("_test."));
        if (testFiles.length > 0) {
            lines.push("", `### 🧪 Test files cần chạy lại`, ...testFiles.map(f => `- \`${f}\``));
        }
...
async function findReverseImports(targetFile, projectRoot, _importPattern, maxDepth, extensions) {
    const results = [];
    const visited = new Set();
    // Extract the file stem for searching (remove extension and path)
    const stem = path.basename(targetFile).replace(/\.[^.]+$/, "");
    async function search(target, currentDepth) {
        if (currentDepth > maxDepth)
            return;
        const extGlob = extensions.map(e => `--include=*${e}`).join(" ");
        try {
            const { stdout } = await execAsync(`grep -rl ${extGlob} "${target.replace(/\.[^.]+$/, "")}" . 2>/dev/null || true`, { cwd: projectRoot, timeout: 15000 });
            ...
            try {
                const { stdout } = await execAsync(`findstr /S /M /C:"${target.replace(/\.[^.]+$/, "")}" ${extensions.map(e => `*${e}`).join(" ")}`, { cwd: projectRoot, timeout: 15000 });
                const files = stdout.trim().split("\n")
                    .filter(Boolean)
...
function extractModule(filePath, stack) {
...
function categorizeFile(filePath, _stack) {
    if (filePath.includes(".spec.") || filePath.includes(".test.") || filePath.includes("_test."))
        return "🧪 Test";
    ...
    if (filePath.includes(".controller."))
        return "🌐 Controller";
    if (filePath.includes(".pipe.") || filePath.includes(".directive."))
        return "🔧 Utility";
    if (filePath.includes(".html") || filePath.includes(".scss"))
        return "🎨 Template";
    return "📄 Other";
}
