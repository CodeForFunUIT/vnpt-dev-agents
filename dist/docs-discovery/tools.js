...
export function registerDocsDiscoveryTools(server) {
    // ── TOOL 1: Scan Project Docs ─────────────────
    server.tool("scan_project_docs", "Quét toàn bộ tài liệu .md trong dự án. " +
        ...
        const formatEntry = (d) => `- ${categoryIcon[d.category] || "📄"} \`${d.relativePath}\` (${d.sizeKb}KB) — ${d.category}`;
        const lines = [
            `# 📚 Project Docs Discovery`,
            `**Project:** \`${projectRoot}\``,
