...
export function registerQualityGateTools(server) {
    server.tool("check_quality_gate", "Kiểm tra code quality trước khi close task: lint, build, test. " +
        ...
        try {
            ...
            const { stdout: diffOut } = await execAsync(`git diff --name-only ${baseBranch}...HEAD`, { cwd: projectRoot });
        ...
        const lines = [
            ...
            `| Check | Status | Errors |`,
            `|---|---|---|`,
            ...checks.map(c => `| ${c.name} | ${c.passed ? "✅ Pass" : "❌ Fail"} | ${c.errorCount} |`),
            uncommittedCount > 0
        ...
        if (!allPassed) {
            lines.push("## ❌ Gate BLOCKED", `**${totalErrors} errors** cần fix trước khi close task.`, "", "### Chi tiết lỗi", ...checks
                .filter(c => !c.passed)
                .map(c => [
                `#### ${c.name} (${c.command})`,
                "```",
...
function countTestFailures(output) {
