...
export function registerMetricsTools(server) {
    // ── TOOL 1: Track metric ──────────────────────
    server.tool("track_metric", "Ghi lại metrics định lượng sau khi hoàn thành một task. " +
        ...
        const ratioNote = Number(estRatio) > 1.3 ? "⚠️ Thực tế lâu hơn ước tính" :
    ...
    server.tool("get_metrics_report", "Tạo báo cáo tổng quan metrics theo khoảng thời gian hoặc sprint. " +
        ...
        // Apply filters
        let entries = [...store.entries].sort((a, b) => new Date(b.recordedAt).getTime() - new Date(a.recordedAt).getTime());
        if (lastNDays) {
        ...
        if (filterTag) {
            entries = entries.filter((e) => e.tags.some((t) => t.toLowerCase().includes(filterTag.toLowerCase())));
        }
        if (entries.length === 0) {
            return {
        ...
        // Format helper
        const fmtTime = (mins) => mins >= 60 ? `${(mins / 60).toFixed(1)}h` : `${Math.round(mins)}m`;
        ...
        const lines = [
            ...
            "",
            "## 📋 Task Type Breakdown",
            ...Object.entries(byType).map(([type, count]) => `- ${type}: ${count} tasks (${pct(count / entries.length)})`),
            "",
        ];
        // Sprint comparison nếu yêu cầu
        if (compareLastNSprints && compareLastNSprints > 1) {
            const allSprints = [...new Set(store.entries.filter((e) => e.sprint).map((e) => e.sprint))].slice(-compareLastNSprints);
    ...
    server.tool("get_metrics_dashboard", "Tạo dashboard HTML visualize metrics inline trong Antigravity. " +
        ...
        const labels = entries.map((e) => new Date(e.recordedAt).toLocaleDateString("vi-VN", { day: "2-digit", month: "2-digit" }));
        ...
        const aiSuccessRate = Math.round((entries.filter((e) => e.aiSucceeded).length / entries.length) * 100);
...
// ─────────────────────────────────────────────
function buildDashboardHtml(days, totalTasks, successRate, avgQuality, avgRevisions, avgEstRatio, labels, qualityData, revisionData, cycleData) {
    return `
